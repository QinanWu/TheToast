...
- (void)__show:(int)second {
    ...
    //[UIView setAnimationDidStopSelector:@selector(__animationDidStop:__finished:__context:showingTime:)];
    [UIView setAnimationDidStopSelector:@selector(__animationDidStop:__finished:__context:showingTime:)];
    self.alpha = 1.0f;
...
- (void)__animationDidStop:(NSString *)animationID __finished:(NSNumber *)finished __context:(void *)context showingTime:(int)second{
    ...
    if ([animationID isEqualToString:@"hide"]) {
        ...
        [NSTimer scheduledTimerWithTimeInterval:second target:self selector:@selector(__hide) userInfo:nil repeats:NO];
    }
}
